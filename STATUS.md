...
**all 12 weeks are built and live** (problems + passcode-locked solutions). The build is not capped
at any week count: the generator, leaderboard, and backend all handle as many weeks as exist, so the
run could be extended further by just adding `week13/`, … the same way as the others.
...
└── weeks/grade4/
    ...
    ├── week1/ … week12/               ← one folder per week, each with:
...
| **Grade 4 — Week 8** | 6 | race winner (**text**=Mindy), Amelia weight aside (num,img), spell WATER paths (num,img), park distance (num,img), Nathan pushups 13×7 (num), grey≠white (**A–E**,img) | `Mindy, 1, 6, 2, 91, D` |
| **Grade 4 — Week 9** | 6 | ant paths A→B (num,img), biggest gift box (**text**=red,img), 3-digit perms of 1,2,3 (num), sum of first 5 evens odd/even (**text**=even), Torey tile designs (num,img), wheel pattern 1–5 (**choice**,img) | `6, red, 6, even, 4, 4` |
| **Grade 4 — Week 10** | 6 | honey jars pigeonhole (num,img), gears turn/jam (**Turns/Jammed buttons**,img), how many even (num), Ellie page 62+50 (num), cubes layer count (num,img), 3-g weights diff (num,img) | `19, jammed, 5, 111, 19, 7` |
| **Grade 4 — Week 11** | 6 | triangles in bag (num), 3-digit odd digits-sum-3 (num), circle pattern 1–8 (**choice**,img), liars/truth-tellers (**Truth-teller/Liar buttons**,img), balance scales (num,img), half-shaded A–E (**A–E**,img) | `4, 201, 6, liar, 75, b` |
| **Grade 4 — Week 12** | 6 | squared-square side (num,img), Sarah's number (num), gears+belt direction (**Clockwise/Counter-clockwise buttons**,img), cuneiform subtract (num,img), odd-one-out 1–6 (**choice**,img), balance mobile heart (num,img) | `15, 39, clockwise, 19, 4, 3` |
...
All weeks scoring live. Several answers are **text/word** (Week 6 "Mike", Week 8 "Mindy", Week 9 "red"/"even",
Week 10 "jammed", Week 11 "liar", Week 12 "clockwise") — the backend compares case/space-insensitively,
so word buttons like `Liar`/`Jammed`/`Clockwise` match lowercase keys.
...
All 12 solution pages exist; each is built by reusing the previous week's solutions page as a template
(figures base64-embedded) and is **passcode-locked** via `gate_solutions.py` (see §6). New for the later
weeks: `build_week.py` supports per-figure `caption` (text between images), a `pill` style for word
choices (e.g. Turns/Jammed), and revealing 💡 hints in the print view.
...
- [x] Problems + **passcode-locked** solution pages built for **all 12 weeks** (`solutions_available: false`).
